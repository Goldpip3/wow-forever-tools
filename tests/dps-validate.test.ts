...
describe('reports that are not, even with a good character inside', () => {
  const broken: Array<[string, (r: any) => void]> = [
    ...
    ['missing histogram minimum', (r) => { delete r.summary.histogram.min; }],
    ['missing histogram maximum', (r) => { delete r.summary.histogram.max; }],
    ['reversed histogram bounds', (r) => { r.summary.histogram.min = 10; r.summary.histogram.max = 1; }],
    ['negative histogram counts', (r) => { r.summary.histogram.bins = [-1]; }],
    ['missing ability damage', (r) => { delete r.summary.abilities[0].damage; }],
    ['missing ability name', (r) => { delete r.summary.abilities[0].name; }],
    ['missing resource values', (r) => { r.summary.resources = {}; }],
    ['null resource value', (r) => { r.summary.resources.timeIdle = null; }],
