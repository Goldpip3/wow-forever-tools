...
export function effectsForSpec(classId: string, specId: number): Effect[] {
  return EFFECTS.filter((e) =>
    e.providers.some((p) => p.classId === classId && (!p.specs || p.specs.includes(specId))),
  );
}

/** Choice groups a spec has to pick from, e.g. which aura or which totems. */
export function choiceGroupsForSpec(classId: string, specId: number): Map<string, Effect[]> {
...
export { WARRIOR_EFFECTS, PALADIN_EFFECTS, HUNTER_EFFECTS, ROGUE_EFFECTS, PRIEST_EFFECTS, SHAMAN_EFFECTS, MAGE_EFFECTS, WARLOCK_EFFECTS, DRUID_EFFECTS };
